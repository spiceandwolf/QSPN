...
If this query workload is only used as update workload, only this one file is enough. But please note that if the $q$-th (staring from $0$) query is an update operation (INSERT sentence), the $\text{'arr_query}[q, :, 1]\text{'}$ must be all $-1$.
